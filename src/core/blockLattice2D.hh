...
{
  std::cout<<"blockLattice2D:: constructor ----------------------1" <<std::endl;
  allocateMemory();
  resetPostProcessors();
...
BlockLattice2D<T,Lattice>::BlockLattice2D(BlockLattice2D<T,Lattice> const& rhs)
  : serializer(0), unSerializer(0),
    dataAnalysis( new DataAnalysis2D<T,Lattice>(*this) )
{
  std::cout<<"blockLattice2D:: constuctor ----------------------2" <<std::endl;
  nx = rhs.nx;
  ny = rhs.ny;
  allocateMemory();
  resetPostProcessors();
...
#pragma noprefetch 
    for (int iY=y0; iY<=y1; ++iY) {
 #if 0 //prefetch 
      _mm_prefetch((char*)&grid[iX][iY+7][0],_MM_HINT_NTA);
      _mm_prefetch((char*)&grid[iX][iY+7][8],_MM_HINT_NTA);
 #endif //prefetch
     grid[iX][iY].collide(getStatistics());// original code
      lbHelpers<T,Lattice>::swapAndStream2D(grid, iX, iY);
  	   //std::cout<<"==============bulk collideAndStream: "<<" ix: "<<iX<<" iY: "<<iY<<std::endl;
    }
}
#else //cache block
int BSX=32;
int BSY=y1-y0;
 for (int iX=x0; iX<=x1; iX+=BSX) {
    for (int iY=y0; iY<=y1; iY+=BSY) {
     int ixmax=iX+BSX; if(ixmax>x1+1) ixmax=x1+1;
     int iymax=iY+BSY; if(iymax>y1+1) iymax=y1+1;
      if(iX+BSX>ixmax || iY+BSY>iymax)
      {
	  for (int iX_=iX; iX_<ixmax; ++iX_)
          for (int iY_=iY; iY_<iymax; ++iY_) {
             grid[iX_][iY_].collide(getStatistics());
             lbHelpers<T,Lattice>::swapAndStream2D(grid, iX_, iY_);
          }
       }
       else
       {
  ...
  int x0, int x1, int y0, int y1 )
{
  ...
  if (omp.get_size() <= x1-x0+1) {
	...
	*/
            ...
            T fTmp                   = grid[iX][iY][1];
            grid[iX][iY][1]       = grid[iX][iY][5];
            grid[iX][iY][5]  = grid[iX-1][iY+1][1];
            grid[iX-1][iY+1][1] = fTmp;
            fTmp                   = grid[iX][iY][2];
            grid[iX][iY][2]       = grid[iX][iY][6];
            grid[iX][iY][6]  = grid[iX-1][iY][2];
            grid[iX-1][iY][2] = fTmp;
            fTmp                 = grid[iX][iY][3];
            grid[iX][iY][3]   = grid[iX][iY][7];
            grid[iX][iY][7] = grid[iX-1][iY-1][3];
            grid[iX-1][iY-1][3]   = fTmp;
            fTmp                 = grid[iX][iY][4];
            grid[iX][iY][4]   = grid[iX][iY][8];
            grid[iX][iY][8] = grid[iX][iY-1][4];
            grid[iX][iY-1][4]   = fTmp;
#endif
        }
      }
...
int BSX=32;
int BSY=y1-y0;
int x1_=loadbalance.get_lastGlobNum();
 for (int iX=loadbalance.get_firstGlobNum()+1; iX<=x1_; iX+=BSX) {
    for (int iY=y0; iY<=y1; iY+=BSY) {
     int ixmax=iX+BSX; if(ixmax>x1_+1) ixmax=x1_+1;
     int iymax=iY+BSY; if(iymax>y1+1) iymax=y1+1;
      if(iX+BSX>ixmax || iY+BSY>iymax)
      {
	  for (int iX_=iX; iX_<ixmax; ++iX_)
...
#else //loop unrolling
          std::swap(grid[iX][iY][5],
                    grid[iX-1][iY+1][1]);
          std::swap(grid[iX][iY][6],
                    grid[iX-1][iY][2]);
          std::swap(grid[iX][iY][7],
                    grid[iX-1][iY-1][3]);
          std::swap(grid[iX][iY][8],
                    grid[iX][iY-1][4]);
#endif
      }
    }
