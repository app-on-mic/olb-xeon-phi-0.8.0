...
struct lbDynamicsHelpers<T, descriptors::D2Q9DescriptorBase<T> > {
  ...
  static T bgkCollision ( T* cell, T rho, const T u[2], T omega) {
    ...
    T uxSqr__ = (T)1.5 * uxSqr;
    T uySqr__ = (T)1.5 * uySqr;
    T uSqr_ = uxSqr__ + uySqr__;

    //T uxPySqr_ = (T)9/(T)2 * (u[0]+u[1])*(u[0]+u[1]);
    //T uxMySqr_ = (T)9/(T)2 * (u[0]-u[1])*(u[0]-u[1]);
    //T uxPySqr_ = (T)4.5 * (u[0]+u[1])*(u[0]+u[1]);
    //T uxMySqr_ = (T)4.5 * (u[0]-u[1])*(u[0]-u[1]);
    T uxPySqr_ = (T)4.5 * (u0+u1)*(u0+u1);
    T uxMySqr_ = (T)4.5 * (u0-u1)*(u0-u1);
   ...
   // cf_  = (T)1/(T)9 * (rho-(T)1);
    ...
    cell[6] *= (T)1-omega;
    cell[6] += (T)0.25*omega*(cf_ + rho_*(ux_ + uxSqr_ - uySqr__));
    cell[8] *= (T)1-omega;
    cell[8] += (T)0.25*omega*(cf_ + rho_*(uy_ + uySqr_ - uxSqr__));
    cell[2] *= (T)1-omega;
    cell[2] += (T)0.25*omega*(cf_ + rho_*(-ux_ + uxSqr_ - uySqr__));
    cell[4] *= (T)1-omega;
    cell[4] += (T)0.25*omega*(cf_ + rho_*(-uy_ + uySqr_ - uxSqr__));

    //rho_ = (T)1/(T)36 * rho;
    //cf_  = (T)1/(T)36 * (rho-(T)1);

    cell[7] *= (T)1-omega;
    cell[7] += (T)0.0625*omega*(cf_ + rho_*(ux_ + uy_ + uxPySqr_ - uSqr_));
    cell[1] *= (T)1-omega;
    cell[1] += (T) 0.0625*omega*(cf_ + rho_*(-ux_ + uy_ + uxMySqr_ - uSqr_));
    cell[3] *= (T)1-omega;
    cell[3] += (T)0.0625*omega*(cf_ + rho_*(-ux_ - uy_ + uxPySqr_ - uSqr_));
    cell[5] *= (T)1-omega;
    cell[5] += (T)0.0625*omega*(cf_ + rho_*(ux_ - uy_ + uxMySqr_ - uSqr_));
  ...
  static void computeRhoU(T const* cell, T& rho, T u[2]) {
    ...
    T rho_ =(T)1/rho;
    u[0]  = (lineX_P1 - lineX_M1) *rho_;
    u[1]  = (lineY_P1 - lineY_M1)*rho_;
  }
